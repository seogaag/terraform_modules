...
def handler(event, context):
    ...
    def create_model(model_name, model_artifact, image_uri):
        response = sagemaker.create_model(
            ModelName=model_name,
            PrimaryContainer={
                'Image': '991648021394.dkr.ecr.ap-south-1.amazonaws.com/forecasting-deepar:latest',
                'ModelDataUrl': f's3://esia-stock-test/model-output/{company}/ESIATrainingJob-{company}-2024-08-22101/output/model.tar.gz'
            },
            ExecutionRoleArn='arn:aws:iam::381492185710:policy/esia-test-sagemaker-policy'
        )
    ...
    for company in companies:
        ...
        create_model(model_name, endpoint_config_name, current_model_endpoint_name)
        create_endpoint(model_name, endpoint_config_name, current_model_endpoint_name)
        ...
        # 새 모델을 평가할 때 사용하는 엔드포인트 이름
        new_model_endpoint_name = f'{company}-new-forecasting-endpoint'
        create_model(model_name, endpoint_config_name, new_model_endpoint_name)
        create_endpoint(model_name, endpoint_config_name, new_model_endpoint_name)
